# 📊 TableToResource 表格资源转换插件指南

**TableToResource** 是一个为 Godot 4.4+ 深度定制的双向数据同步插件。它不仅能将 CSV 电子表格一键批量转换为 Godot 原生的 `.tres` 资源文件，还能将游戏中调整好的资源文件逆向导出为 CSV 表格。
...
1. 点击插件界面中的 **[扫描]** 按钮。
2. 选择你存放 GDScript 数据类（`.gd`）的根目录。
3. 插件会自动提取所有带有 `class_name` 的脚本，并在插件核心目录下生成 `class_map.json` 供运行时查表使用。
...
1. 切入 **[逆向导出]** 选项卡。
2. **选择资源目录**：选择包含 `.tres` 文件的文件夹。
3. **另存为 CSV**：设置导出的表格路径和文件名（如 `res://data/csv/items_export.csv`）。
4. 点击 **[导出 CSV 表格]**。插件会自动清洗底层属性，将你的枚举、套娃资源等“降维”还原为高度可读的 CSV 格式。

---

## 📝 电子表格数据填写规范 (约定优于配置)

插件采用**强类型反射机制**。你不需要在表头定义类型，插件会自动读取关联的 `.gd` 脚本中的变量类型，对 CSV 单元格内容进行强制转换。
...
> **提示**：插件通过全局常量池智能反查，确保无论填数字还是字符串，正逆向都能完美对应。
...
> **提示**：`(0, 1)` 会被自动解析为 `Vector2`，`(0, 0, 0)` 会被解析为 `Vector3`。
